...
class BalanceTransferDatatable extends BaseDatatable
{
    ...
    public function configure($datatable): DataTableAbstract
    {
        ...
        ->addColumn('action', function ($balanceTransfer) {
            $action = view('components.link', ['href' => route('superdistributor.balance-transfers.show', $balanceTransfer->id), 'label' => 'show', 'class' => 'btn btn-sm btn-info']);
            
            if ($balanceTransfer->deleted_at == null)
                $action = $action .
                    view('components.link', ['href' => route('superdistributor.balance-transfers.edit', $balanceTransfer->id), 'label' => 'edit', 'class' => 'btn btn-sm btn-warning']) .
                    view('components.user-btn-delete', ['url' => route('superdistributor.balance-transfers.destroy', $balanceTransfer->id)]);
