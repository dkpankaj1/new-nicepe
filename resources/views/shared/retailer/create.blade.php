<x-app-layout>
    ...
    @section('page-title', 'Create Retailer')
    @section('breadcrumb')
        {!! $breadcrumb !!}
    @endsection
    ...
    <div class="container-fluid">
        <form action="{{ route('superdistributor.retailers.store') }}" method="post">
            @csrf
            <div class="card">
                    ...
                    <div class="row">
                            ...
                            <select name="state" class="form-control">
                                ...
                                @foreach ($country->states as $state)
                                    <option value="{{ $state->name }}"
                                        @if ($state->name === old('state')) selected @endif>
                                        {{ $state->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="plan" class="form-control">
                                ...
                                @foreach ($plans as $plan)
                                    <option value="{{ $plan->id }}"
                                        @if (old('plans') == $plan->id) selected @endif>{{ $plan->name }}</option>
                                @endforeach
                            </select>
                            @error('plan')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="is_active" class="form-control">
                                ...
                                <option value="0" @if (old('is_active') == '0') selected @endif>in-active
                                </option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
