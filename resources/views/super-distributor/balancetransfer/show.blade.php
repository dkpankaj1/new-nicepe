<x-app-layout>
    ...
    @section('breadcrumb', Breadcrumbs::render('superdistributor.balance-transfers.show', $balanceTransfer))
    <!-- Start Content-->
        ...
        <div class="card-body">
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <p class="mb-0"><strong>Email:</strong> {{ $balanceTransfer->fromUser->email }}</p>
                    <p><strong>Name:</strong> {{ $balanceTransfer->fromUser->name }}</p>
                </div>
                <div class="col-md-6">
                    <h6 class="text-primary">To Account</h6>
                    <p class="mb-0"><strong>Email:</strong> {{ $balanceTransfer->toUser->email }}</p>
                    <p><strong>Name:</strong> {{ $balanceTransfer->toUser->name }}</p>
                </div>
            </div>
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <p><strong>Status:</strong> <span
                            class="badge bg-success">{{ ucfirst($balanceTransfer->transaction->status) }}</span></p>
                </div>
                <div class="col-md-6">
                    <h6 class="text-primary">Amount Details</h6>
                    <p><strong>Amount:</strong> {{ number_format($balanceTransfer->transaction->amount, 2) }}
                        {{ $balanceTransfer->transaction->currency->code }}
                    </p>
                    <p><strong>Net Amount:</strong> {{ number_format($balanceTransfer->transaction->amount, 2) }}
                        {{ $balanceTransfer->transaction->currency->code }}
                    </p>
                </div>
            </div>
            ...
            <div class="row mb-4">
                <div class="col-md-6">
                    ...
                    <p><strong>Opening Balance:</strong>
                        {{ number_format($balanceTransfer->transaction->opening_balance, 2) }}  {{ $balanceTransfer->transaction->currency->code }}</p>
                    <p><strong>Closing Balance:</strong>
                        {{ number_format($balanceTransfer->transaction->closing_balance, 2) }}  {{ $balanceTransfer->transaction->currency->code }}</p>
                </div>
                <div class="col-md-6">
                    ...
                    <p><strong>Transaction Type:</strong> {{ ucfirst($balanceTransfer->transaction->transaction_type) }}
                    </p>
                </div>
            </div>
