<x-app-layout>
    ...
    @section('page-title', 'Create Rewards')
    @section('breadcrumb', Breadcrumbs::render('admin.rewards.create'))
                ...
                <form action="{{ route('admin.rewards.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" name="point" class="form-control"
                            placeholder="Enter Reward Point" value="{{ old('point') }}">
                        @error('point')
                            <span class="invalid-feedback d-block text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="reward_type" class="form-control">
                            <option value="">---select---</option>
                            <option value="0" @if (old('reward_type') == 0) selected @endif>Fixed</option>
                            <option value="1" @if (old('reward_type') == 1) selected @endif>Percentage</option>
                        </select>
                        @error('reward_type')
                            <span class="invalid-feedback d-block text-danger">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="expiry_date" class="form-control" id="name" value="{{ old('expiry_date') }}">
                        @error('expiry_date')
                            <span class="invalid-feedback d-block text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="enable" class="form-control">
                            <option value="">---select---</option>
                            <option value="1" @if (old('enable') == 1) selected @endif>Yes
                            </option>
                            <option value="0" @if (old('enable') == 0) selected @endif>No
                            </option>
                        </select>
                        @error('enable')
                            <span class="invalid-feedback d-block text-danger">{{ $message }}</span>
